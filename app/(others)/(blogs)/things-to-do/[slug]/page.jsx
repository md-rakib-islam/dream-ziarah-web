...
const SingleBlogPage = dynamic(() => import("@/components/blogs/singleBlogPage/SingleBlogPage"), {
  ssr: true,
  loading: () => (
    <div className="things-to-do-skeleton" style={{ minHeight: '80vh', padding: '40px 0' }}>
...
export async function generateMetadata({ params }) {
  ...
  try {
    ...
    if (blogData && blogData.meta_title && blogData.title?.toLowerCase().includes('things to do')) {
      return {
        metadataBase: new URL('https://dreamtourism.it'),
        title: blogData.meta_title,
        description: blogData.meta_description,
        keywords: blogData.keywords || `${blogData.title}, things to do, Italy activities, ${slug.replace(/-/g, ' ')}`,
        
        // Enhanced Open Graph
        openGraph: {
          title: blogData.meta_title,
          description: blogData.meta_description,
          url: `https://dreamtourism.it/things-to-do/${slug}`,
        ...
        // SEO Enhancements
        alternates: {
          canonical: `https://dreamtourism.it/things-to-do/${slug}`
        },
...
const ThingsToDoPost = async ({ params }) => {
  const { slug } = await params;
  const fullUrl = `https://dreamtourism.it/things-to-do/${slug}`;
  ...
  try {
    ...
    if (blogContent && blogContent.title && blogContent.title.toLowerCase().includes('things to do')) {
      // Parse HTML on server-side for Edge Runtime compatibility
      const { headings, firstParagraph, updatedHTML } = parseBlogHTML(blogContent.description);
            ...
            fallback={
              <div className="container py-5">
                <div className="text-center">
                  ...
                  <p className="text-muted">This activity guide may have been moved or is temporarily unavailable.</p>
